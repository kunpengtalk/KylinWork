...
| 你平时卡在哪 | KylinWork 怎么处理 |
| --- | --- |
| 「AI 写的报告，格式还得我自己排」 | 直接产出可打开的 `.docx` / `.xlsx` / `.pptx` / `.pdf`，图表是渲染好的图片，不是占位符 |
| 「任务跑一半窗口关了，白干」 | 任务在后台跑，关窗口、切页面、重开应用都不中断，回来自动接回直播 |
| 「不知道它到底动了什么文件」 | 成果面板实时列产物、标出本轮新增；全程有审计，命令与写文件看得见 |
| 「怕它在我机器上乱来」 | 四档权限 + 文件黑名单 + 高危命令二次确认，默认只在工作目录内动手 |
| 「模型换来换去，Key 存哪」 | 十家服务商内置，填 Key 即用、当场验活；Key 只留在本机 `config.json` |
| 「资料一堆，它答得不准」 | 知识库 RAG：向量召回 + 重排，回答时自动引用你自己的文档 |
| 「每次都要重新交代一遍要求」 | 专家预设人格与技能，专家团可一次派整团接力；偏好与记忆长期生效 |

---

## 界面预览

**任务首页——说一句话就能开工。** 需求直达，常用场景一键起手，`@` 引用工作区文件、`/` 调用技能：
...
**设置 · 知识库——向量召回 + 重排，切块与召回参数都可调。** 库与资料在「知识库」页管理：

![知识库设置](./docs/images/ui-settings-knowledge.png)

**设置 · 专家——角色设定 + 专属技能，任务里可被委派。** 内置十位，存本机，不登录也能增删：

![专家设置](./docs/images/ui-settings-experts.png)

**设置 · 频道——微信扫码、飞书、QQ、企业微信接入，在 IM 里直接对话。** 不需要公网地址，手机上发一句话，任务在电脑上跑：

![频道设置](./docs/images/ui-settings-channels.png)
...
「这次用了哪条向量模型、走没走向量」。切块大小、重叠、`top_k`、相似度阈值与注入上限都能在
「设置 → 知识库」里调。
...
把 IM 接上（微信扫码 / 飞书 / 企业微信 / 公众号 / QQ / Webhook），在手机上发一句话，任务就在电脑上跑，
跑完把结果推回聊天窗口——**不需要公网地址**。桌面版窗口失焦时，任务完成、等待审批、出错都会弹系统通知，
点通知能把窗口拉回来。
...
<details>
<summary><b>换过运行方式后，旧会话打不开？</b></summary>

历史列表存在浏览器本地，而会话正文按数据目录存。桌面版（`~/KylinWork`）与源码跑（仓库目录）
用的是两个目录，所以在一边开的会话在另一边找不到——换回原来那个数据目录即可，内容没丢。
</details>
